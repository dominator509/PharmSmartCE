...
- [ ] Test category audited (`verify.sh` + coverage gate + golden-set)
- [ ] Security category audited (`security-check.sh` + `dependency-audit.sh` + authz matrix)
...
- [ ] Accessibility audited (axe-core no `serious` violations on listed pages)
