...
- Constrained answer submissions to non-negative indices in `AnswerDTO` so negative choices now fail at the request boundary, then regenerated `apps/api/openapi.json` and added a focused DTO regression test.
...
- Constrained source upload filenames to 255 characters in the course service and `SourceDTO`, then regenerated `apps/api/openapi.json` so the committed snapshot matches the new contract width.
- Hardened `apps/api/app/adapters/storage/faiss_store.py` to reject malformed on-disk metadata rows instead of assuming every JSON line has the expected `chunk_id` and `metadata` shape; added a malformed-metadata regression test.
